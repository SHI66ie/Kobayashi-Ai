...
2. **Context Assembly**: Simulation variables (aero, fuel, MGU-K) are encoded into context.
3. **Historical DNA Integration**: The engine scans `Data/f1-telemetry/` for matching track results (2020-2025) to provide real-world baseline constraints.
4. **Inference**: The model evaluates conditions against 2026 technical regulations in real-time.
5. **Scoring**: A confidence `ALPHA` score is calculated based on historical validation.
...
4. **Archive Training**: Add new JSON datasets to `Data/f1-telemetry/` to improve the AI's "Historical IQ".
